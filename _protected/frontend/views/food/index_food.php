...
$this->registerJs(" function ff(id,price) {
  ...
  if(amount > 0){
      if(roomId > 0){
            window.location.href = \" " . \yii\helpers\Url::to(['paypal/paypalfood']) . "?foodId=\"+foodId+\"&price=\"+price+\"&amt=\"+amount+\"&roomId=\"+roomId+\"&payfrom=food\";
      }else{
            window.location.href = \" " . \yii\helpers\Url::to(['paypal/paypalfood']) . "?foodId=\"+foodId+\"&price=\"+price+\"&amt=\"+amount+\"&roomId=0&payfrom=food\";
      }
...
$this->registerJs(" function exportPdf(id,price) {
   console.log('ff worked! with foodId -> '+id);
  var foodId = id;
  var price = price;
  var amount = $(\"input[name=amt\"+id+\"]\").val();
  ...
  if(amount > 0){
      if(roomId > 0){
          var url = \" " . \yii\helpers\Url::to(['food/paymoney'])
        . "?foodId=\"+foodId+\"&price=\"+price+\"&amt=\"+amount+\"&roomId=\"+roomId;
        
            //window.open(url, '_blank');
            window.open(url, '_self');
      }else{
         var url = \" " . \yii\helpers\Url::to(['food/paymoney'])
        . "?foodId=\"+foodId+\"&price=\"+price+\"&amt=\"+amount+\"&roomId=0\";
...
<div class="food-index">
    ...
    <?= Html::a('กลับ', ['orders/index'], ['class' => 'btn btn-default']) ?>
    <div class="row">
    ...
    padding-left: 30px;">ยกเลิก
                                ...
                                <button class="btn btn-info btn-lg btn-block btn-capital" name="pay<?= $model['Fname'] ?>"
                                ...
                                <button class="btn btn-success btn-lg btn-block btn-capital" name="pay<?= $model['Fname'] ?>"
                                        onclick="exportPdf(<?= $model['Fid'] . ',' . ($model['Fprice']) ?>)" style="    margin: 10px 1px;
                                    padding-left: 30px;
                                    width: 100%;">
                                    <i class="fa fa-money"
